...
function Slide({ location, close, isAnimating, onNext, onPrevious }) { 
  ...
  return (
    <div className="fixed inset-0 z-40" onClick={close}>
      <div
        className={`fixed inset-y-0 left-0 bg-grainbkg-100 w-screen   md:full lg:w-3/5 max-w-5xl transition-transform duration-300 ease-in-out ${
      ...
      <div className="p-3 sm:p-4 md:p-6 lg:p-8 relative z-30 flex flex-col min-h-full justify-between "> 
        ...
        <div className='flex flex-col justify-between'>
          ...
          <div className="flex flex-col justify-between">
            ...
            <div className="flex flex-row items-center gap-2">
              ...
              <h2 className="text-sm sm:text-xl md:text-3xl lg:text-[3rem] special-elite text-primary-100 ">
            ...
            <div className="flex flex-col justify-between gap-6 sm:gap-8 md:gap-10"> 
              ...
              {/* the Album and there three button for bigger screens */}
              <div className="flex flex-row gap-4 sm:gap-6 hidden md:hidden lg:flex">
                <img
                  src={AlbumImg}
                  alt="album cover"
                  className="w-[344px] sm:w-48 md:w-52 lg:w-70 xl:w-90"
                />
                <div className="flex flex-col gap-1 sm:gap-1.5 justify-between flex-wrap"> 
              ...
              {/* the Album and there three button for bigger screens */}
               {/* the Album and there three button for small screens */}
              <div className="flex flex-col gap-4 sm:gap-6  lg:hidden">
            
                <div className="flex flex-col gap-1 justify-between flex-wrap"> 
                        ...
                        <button className="group relative flex items-center gap-6  justify-between py-2 px-4 sm:px-6 rounded-[10px] w-full cursor-pointer transition-all duration-300 hover:bg-surface-100 focus:bg-secondy-100 outline-none">
                          ...
                          <div className="flex items-center gap-2 z-10">
                            <img
                              src={btn.icon}
                              alt={`${btn.text} icon`}
                              className="w-6 h-4 sm:w-8 sm:h-8 transition-colors duration-100 group-hover:brightness-0 group-hover:invert group-focus:invert group-focus:brightness-0" 
                            />
                            <span className="rock text-[0.75rem] sm:text-[0.875rem] md:text-[1rem] tracking-wider text-secondy-100 transition-colors duration-100 group-hover:text-white group-focus:text-white font-biro"> 
                              {btn.text}
                            </span>
        ...
        <div className="   bg-grainbkg-100 py-4 flex flex-row items-center gap-2 sm:gap-4"
          style={{ backgroundImage: `url(${grainBG})` }}
        >
          <button onClick={onPrevious} type="button" className='group relative transition-all w-[80px] sm:w-[100px] h-[40px] sm:h-[40px] p-0.5 px-1 pt-[3px] duration-300'>
          ...
          <button onClick={onNext} type="button" className='group relative transition-all h-[40px] sm:h-[40px] p-0.5 px-1 pt-[3px] duration-300'> 
            <img src={previousBtn} alt="Button Border Frame" className='absolute left-0 top-0 w-full h-full' />
...
//  // new slide component for on click zoom in map
// import KExpWithCloseBtnHeadingCream from '../components/KExpWithCloseBtnHeadingCream';
// import subwayloc from '../assets/icons/subway_location_Icon.svg';
// import grainBG from '../assets/backgrounds/grainBG.png';
// import AlbumImg from '../assets/favicon.jpg';
...
//     // prefer selectedSong (when user clicked a musical note), otherwise use first song
//   const song = location.selectedSong || (location.songs && location.songs[0]) || null;
